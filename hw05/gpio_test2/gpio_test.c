...
// gpio_set_value(gpioLED, ledOn);          // Not required as set by line above (here for reference)
   ...
   result = request_irq(irqNumber1,             // The interrupt number requested
                        ...
                        IRQF_TRIGGER_RISING,   // Interrupt on rising edges (button press, not release)
 ...
 */
static irq_handler_t ebbgpio_irq_handler1(unsigned int irq1, void *dev_id, struct pt_regs *regs){
...
static irq_handler_t ebbgpio_irq_handler2(unsigned int irq2, void *dev_id, struct pt_regs *regs){
   ledOn2 = !ledOn2;
